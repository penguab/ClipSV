#!/usr/bin/env python	
def merge_large_insertion(large_insertion):
	...
	def comp(para1,para2):
		...
		if line[0]==region[0] and int(line[2])-int(region[2])<=20:
			...
			out=para2.split('\t')
			if int(out[5])>=3 and (int(out[9])>=3 or int(out[10])>=3):
	...
	if left!='':
		out_left=left.split('\t')
		if int(out_left[5])>=3 and (int(out_left[9])>=3 or int(out_left[10])>=3):
	...
	if right!='':
		...
		if int(out_right[5])>=3 and (int(out_right[9])>=3 or int(out_right[10])>=3):
